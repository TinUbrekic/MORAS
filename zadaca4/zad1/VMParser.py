...
class Parser:
    ...
    def _parseLines(self):
        pat = '^\s*label \w+|W+$'
        for (l,n) in self._ilines:
            if bool(re.search(pat, str(l))):
                lajn = re.search(pat, str(l)).group()
                self._labels.append(lajn.split()[1])
    ...
    def _goto(self, lab, n):
        if lab not in self._labels:
            self._flag = False
            Parser._error("goto", n, "Trying to jump to undefined label");
            return ""
        return "@" + self._func + "$" + lab + "\n0;JMP"
    ...
    def _ifgoto(self, lab, n):
        if lab not in self._labels:
            self._flag = False
            Parser._error("if-goto", n, "Trying to jump to undefined label");
            return ""
        return "@SP\nAM=M-1\nD=M+1\n@" + self._func + "$" + lab + "\nD;JEQ"
    ...
    def _push(self, src, loc, n):
        ...
        if not bool(re.search(pattern, test)):
            self._flag = False
            Parser._error("Push", n, "non-integer as push argument"); 
            return ""
    ...
    def _pop(self, dst, loc, n):
        ...
        if not bool(re.search(pattern, test)):
            self._flag = False
            Parser._error("Pop", n, "non-integer as pop argument"); 
            return ""
